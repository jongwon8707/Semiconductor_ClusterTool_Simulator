...
class Agent():
    ...
    def run(self, trained_model_file='', is_training=True, render=False):
        ...
        for episode in itertools.count():
            ...
            print(f"Episode {episode}, Reward: {episode_reward}, info: {info}, Epsilon: {epsilon:.2f}")
    ...
    def evaluate(self, num_episodes=10, render=False):
        ...
        model1_path = './환경C_0.99_1405/eqp-scheduler-v0.pt'
        model2_path = './환경C_0.9995_47716/eqp-scheduler-v0.pt'
        model3_path = './환경A_0.9995_40692/eqp-scheduler-v0.pt'
        ...
        for _ in range(num_episodes):
            ...
            random_wafer = self.run_episode(env, policy_dqn = policy_dqn_model4, use_model=False)
        ...
        env.close()
        return model1_wafers, model2_wafers, model3_wafers, random_wafers, rule1_wafers
    ...
    def compare_and_plot(self, num_episodes=10):
        model1_wafers, model2_wafers, model3_wafers, random_wafers, rule1_wafers = self.evaluate(num_episodes)

        plt.figure(figsize=(10, 6))
        sns.boxplot(data=[model1_wafers, model2_wafers, model3_wafers, random_wafers, rule1_wafers], width=0.6)
        plt.xticks([0, 1, 2, 3, 4], ['Model (Env C) w/ 0.99', 'Model (Env C) w/ 0.9995','Model (Env A) w/ 0.99', 'Random', 'Rule1'])
        plt.ylabel('Total Wafers Processed per Episode')
        plt.savefig('model_vs_random_comparison.png')
        plt.close()

        print(f"Model (Env C) w/ 0.99 - Mean: {np.mean(model1_wafers):.2f}, Std: {np.std(model1_wafers):.2f}")
        print(f"Model (Env C) w/ 0.9995 - Mean: {np.mean(model2_wafers):.2f}, Std: {np.std(model2_wafers):.2f}")
        print(f"Model (Env A) w/ 0.99 - Mean: {np.mean(model3_wafers):.2f}, Std: {np.std(model3_wafers):.2f}")
...
if __name__ == '__main__':
    ...
    dql.compare_and_plot(num_episodes=5)