...
class EQP_Scheduler:
    ...
    def __init__(self, MAX_WAFER_NO):
        ...
        self.CMB_MinExecutionTime = 60
        self.CMB_MaxExecutionTime = 65
        self.EntryExitWaitTime = 3
        self.CMD_CLEANING_LIMIT = 200
        self.CMD_CLEANING_TIME = 10
    ...
    def reset(self, seed=None):
        ...
        for component_list in self.components.values():
            if isinstance(component_list, list):
                ...
                component_list.cumulative_process_time = 0
                component_list.total_cumulative_process_time = 0
                component_list.processed_wait_time = 0
    ...
    def check_action_validity(self, vtr_action: VTRAction):
        ...
        valid_actions = [
            VTRAction.NO_ACTION,
            VTRAction.ENTRY_TO_LEFT_ARM,
            VTRAction.LEFT_ARM_TO_CHAMBER_1,
            VTRAction.LEFT_ARM_TO_CHAMBER_2,
            VTRAction.LEFT_ARM_TO_CHAMBER_3,
            VTRAction.LEFT_ARM_TO_CHAMBER_4,
            VTRAction.CHAMBER_1_TO_LEFT_ARM,
            VTRAction.CHAMBER_2_TO_LEFT_ARM,
            VTRAction.CHAMBER_3_TO_LEFT_ARM,
            VTRAction.CHAMBER_4_TO_LEFT_ARM,
            VTRAction.LEFT_ARM_TO_EXIT
        ]

        if vtr_action not in valid_actions:
            return False
        ...
        if vtr_action == VTRAction.NO_ACTION:
            ...
            at_least_one_chamber_empty = any(chamber.holding_wafer is None for chamber in chambers[:4])
    ...
    def perform_action(self, vtr_action: VTRAction):
        ...
        # 각 Chamber의 processed wafer 대기 시간 업데이트
        processed_wait_time = 0
        for chamber in self.components[ComponentType.CHAMBER]:
	...
	# Handle exit wait time and remove the wafer from exit
        ...
        if vtr_action == VTRAction.NO_ACTION:
            return 0, wafer_processed, busy_chamber_cnt, processed_wait_time
    ...
    def render(self, cumulative_reward):
        ...
        def draw_module(position, component, text, is_chamber=False):
            color = empty_color
            wafer_id_text = ""
            process_time_text = ""
        ...
        for i, pos in enumerate(chamber_positions):
            draw_module(pos, self.components[ComponentType.CHAMBER][i], f"Chamber {i+1}", is_chamber=True)
